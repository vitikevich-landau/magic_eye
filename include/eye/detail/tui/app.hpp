...
inline const char* gl_rule()   { return gl("─", "-"); }     // полоса ярлыков
inline const char* gl_chip_l() { return gl("▐", "["); }     // край ярлыка слева
inline const char* gl_chip_r() { return gl("▌", "]"); }     // край ярлыка справа
...
struct Layout {
    ...
    std::size_t tabs_y = 0;        // строка ярлыков зон (0 — полосы нет)
...
inline Layout compute_layout(TermSize s) {
    ...
    if (s.cols >= 100 && det_w >= MIN_FRAME_W + 4) {
        ...
        l.det_w = det_w;
        // Полоса ярлыков зон — только здесь: лишь в широком режиме Tab и правда
        // переключает фокус между ДВУМЯ видимыми зонами, и только тут ярлык
        // отвечает на вопрос «где я». Тело съезжает на строку вниз.
        l.tabs_y = 1;
        l.body_y = 2;
        l.body_h = s.rows > 3 ? s.rows - 3 : 1;
    } else {
        l.wide = false;
        l.tree_w = s.cols;
        l.det_x = 0;
...
// ─── ярлык зоны (полоса вкладок над зонами) ──────────────────────────────────
// Отвечает на «где я»: активная зона — инверсией, как курсор дерева; спящая —
// тускло. Сигнал продублирован ГЛИФАМИ и регистром (▐ ВЕРХНИЙ ▌ против
// ── строчного), поэтому читается и при EYE_COLOR=0, где инверсии нет вовсе.
// loud/quiet — две формы имени: кириллицу не поднять в регистр по месту.
inline StyledLine zone_tab(const char* loud, const char* quiet, bool active,
                           std::size_t width) {
    std::string chip = active
        ? std::string(gl_chip_l()) + " " + loud + " " + gl_chip_r()
        : std::string(gl_rule()) + gl_rule() + " " + quiet + " ";
    std::size_t w = vwidth(chip);
    if (w > width) {                       // зона уже ярлыка — режем по бюджету
        chip = clip(chip, width);
        w = vwidth(chip);
    }
    std::string s = active
        ? std::string(clr::code("\x1b[7m")) + chip + clr::reset()
        : std::string(clr::dim()) + chip + clr::reset();
    if (w < width) {                       // добить полосу до края зоны
        std::string tail;
        for (std::size_t i = w; i < width; ++i) tail += gl_rule();
        s += std::string(clr::grey()) + tail + clr::reset();
        w = width;
    }
    return {s, w};
}
...
inline StyledLine tree_row_line(const TreeItem* it, bool cursor,
    ...
    if (cursor) {
        ...
        // вложенные reset'ы не разрывали фон. Стрелка ► дублирует курсор там,
        // где цветов нет (EYE_COLOR=0, снапшот-тесты). Когда фокус ушёл в
        // детали (Tab), дерево «припарковано»: стрелка полая ▹ и инверсия
...
inline void draw(App& a, Canvas& canvas, const Layout& l) {
    ...
    // Полоса ярлыков зон: главный ответ на «где я» после Tab.
    if (l.wide) {
        canvas.blit(l.tabs_y, 0,
                    zone_tab("ДЕРЕВО", "дерево", !a.focus_detail, l.tree_w),
                    l.tree_w);
        canvas.blit(l.tabs_y, l.tree_w,
                    StyledLine{std::string(clr::grey()) + gl_spine() +
                                   clr::reset(),
                               1},
                    1);
        canvas.blit(l.tabs_y, l.det_x,
                    zone_tab("ДЕТАЛИ", "детали", a.focus_detail, l.det_w),
                    l.det_w);
    }
